...
onFirebaseReady(() => {
  ...
  auth.onAuthStateChanged(user => {
    ...
    // Cache admin status for site-settings.js (page-visibility guard).
    if (!user) {
      try { localStorage.removeItem('gk_is_admin'); } catch (e) {}
    } else if (window.GK && GK.getUserRecord) {
      GK.getUserRecord(user.uid).then(rec => {
        try { localStorage.setItem('gk_is_admin', rec && rec.isAdmin ? '1' : '0'); } catch (e) {}
      }).catch(() => {});
    }
